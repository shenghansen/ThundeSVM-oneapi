...
void SVC::train(const DataSet &dataset, SvmParam param) {
    ...
    vector<SyncArray<float_type>> alpha(n_binary_models);//拉格朗日乘子
    vector<bool> is_sv(dataset_.n_instances(), false);//记录每个instance是否为支持向量

    int k = 0;
    for (int i = 0; i < n_classes; ++i) {
        for (int j = i + 1; j < n_classes; ++j) {//其实针对二分类问题，只循环一次
...
void SVC::train_binary(const DataSet &dataset, int i, int j, SyncArray<float_type> &alpha, float_type &rho) {
    ...
    SyncArray<int> y(ins.size());//yi
    alpha.resize(ins.size());//拉格朗日乘子
    SyncArray<float_type> f_val(ins.size());//每个样本的指示变量f
    alpha.mem_set(0);
